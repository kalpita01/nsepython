...
# Getting the Case stock details of a stock
...
def get_fut_stock_details(stock_name_lower, parameter):
    stock_name = stock_name_lower.upper()
    if parameter == "This Week":
        try:
            future_stock = nse.get_quote(
                stock_name, segment=Segment.FUT, expiry=dt.date(2021, 5, 20))
            return future_stock
        except:
            return 0
...
# Printing the stock details for top gain and loss
...
# Choosing this week or month menu


def week_or_month():
    markup = types.ReplyKeyboardMarkup(row_width=1, one_time_keyboard=True)
    itembtn1 = types.KeyboardButton('This Week')
    itembtn2 = types.KeyboardButton('This Month')
...
@bot.message_handler(regexp="For Cash")
def cash_stock_details(message):
    global stock_code_global
    data = stock_code_global
    stock_details = get_stock_details(data)
    if stock_details == 0:
        bot.send_message(message.chat.id, "Please enter a valid stock code")
    else:
...
@bot.message_handler(regexp="For Future")
def cash_stock_details(message):
    menu = week_or_month()
    bot.send_message(message.chat.id, "Choose an option", reply_markup=menu)
...
@bot.message_handler(regexp="This Week")
@bot.message_handler(regexp="This Month")
@bot.message_handler(regexp=r"^\/[a-zA-Z]+\s[0-9]{4}\s[0-9]+")
def fut_stock_details(message):
    temp = message.text
    try:
        global stock_code_global
        stock_name = stock_code_global
        stock_details = get_fut_stock_details(stock_name, temp)
        if stock_details == 0:
            bot.send_message(
